...
path_xml = '/home/disorn/code_save/Yolo2Pascal-annotation-conversion/test_depth_fillter/'
...
def write_txt(gt,file_name_text):
    path_gt = '/home/disorn/metrics_measurement/test_depth_fillter/detections/'
    with open(path_gt + file_name_text, 'w') as f:
        for row in gt:
            f.writelines(' '.join(map(str, row)) + '\n')
            
def write_img(img,filename):
    path_save_result = '/home/disorn/metrics_measurement/test_depth_fillter/result_image/'
...
def extract_from_xml(file_to_process):
    ...
    image_BGR = cv2.imread(path_xml + file_name , cv2.IMREAD_UNCHANGED) 
    ...
    network = cv2.dnn.readNetFromDarknet(path+'test1/depthmap_fillter_combine_2color.cfg',
                                        path+'test1/depthmap_fillter_combine_2color_final.weights')
