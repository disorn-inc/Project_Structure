...
path_xml = '/home/disorn/code_save/Yolo2Pascal-annotation-conversion/real_final_rgbd/'
...
def write_txt(gt,file_name_text):
    path_gt = '/home/disorn/metrics_measurement/real_final_rgbd/detections/'
    with open(path_gt + file_name_text, 'w') as f:
        for row in gt:
            f.writelines(' '.join(map(str, row)) + '\n')
            
def write_img(img,filename):
    path_save_result = '/home/disorn/metrics_measurement/real_final_rgbd/result_image/'
...
def extract_from_xml(file_to_process):
    ...
    network = cv2.dnn.readNetFromDarknet(path+'test1/rgbd_realfinal.cfg',
                                        path+'test1/rgbd_realfinal_final.weights')
